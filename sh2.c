...
int sh(int argc, char **argv, char **envp)
{
	...
	while (go)
	{
        ...
        if ((char)args[0][0] == '/') {
          commandpath = malloc(32*sizeof(char));
          strcpy(commandpath, args[0]);
        }
        else if ((char)args[0][0] == '.') {
          if ((char)args[0][1] == '/') {
            commandpath = strcat(getcwd(NULL, PATH_MAX+1), &args[0][1]);
          }
          else if ((char)args[0][1] == '.' && (char)args[0][2] == '/') {
            chdir("..");
            commandpath = strcat(getcwd(NULL, PATH_MAX+1), &args[0][2]);
            chdir(pwd);
          }
        }
        //Otherwise, find the executable
        else {
          commandpath = where(args[0], pathlist);
        }
        ...
        if (commandpath) {
          ...
          if ((pid = fork()) < 0) {
            ...
            int wildcard = 1;
            //Check for and handle wildcards
            while (wildcard) {
              for (int i = 1; i < MAXARGS; i++) {
                if ( strstr(args[i], "*") || strstr(args[i], "?") )  {
                  glob_t globArr;
                  glob(args[i], GLOB_NOCHECK, 0, &globArr);
                  for (int j = 0; j < globArr.gl_pathc; j++) {
                    args[i + j] = malloc(128 * sizeof(char));
                    strcpy(args[i + j], globArr.gl_pathv[j]);
                  }
                  globfree(&globArr);
                  wildcard = 0;
                }
